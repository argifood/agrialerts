	...
	<?php }
	$attributes = array('class' => 'form', 'id' => 'update-alert_dim');
	echo form_open(site_url('alert_dims/update/'.$row['alert_dim_id']), $attributes);
	?>
	<div class='row'>
	<h2 class='text-center'><?php echo $this->lang->line('alert_dim');?></h2>
        ...
        <select class='form-control' name='alerttype_id' id='alerttype_id' required>
          ...
          <option value='<?php echo $key ?>' <?php echo ($key == $row['alerttype_id'])? 'selected' : '' ?>> <?php echo $val ?> </option>
          <?php } ?>
        </select>
      </div> 
        ...
        <select class='form-control' name='agritype_id' id='agritype_id' required>
          ...
          <option value='<?php echo $key ?>' <?php echo ($key == $row['agritype_id'])? 'selected' : '' ?>> <?php echo $val ?> </option>
          <?php } ?>
        </select>
      </div> 
<div class='form-group col-lg-4 col-md-4 col-sm-4 col-xs-12'> 
        ...
        <input type='text' class='form-control' name='agri_dim_severity' id='agri_dim_severity' value='<?php echo $row['agri_dim_severity'] ?>'required />
      </div> 
<div class='form-group col-lg-4 col-md-4 col-sm-4 col-xs-12'> 
...
<div class='form-group col-lg-4 col-md-4 col-sm-4 col-xs-12'> 
        ...
        <select class='form-control' name='nuts5_id' id='nuts5_id' required>
          <option value=''><?php echo $this->lang->line('nuts5_id') ?></option>
          <?php foreach($nuts5 As $key => $val) { ?>
          <option value='<?php echo $key ?>' <?php echo ($key == $row['nuts5_id'])? 'selected' : '' ?>> <?php echo $val ?> </option>
          <?php } ?>
        </select>
      </div> 
	...
	<div class='row'>
		<div class='form-group col-lg-6 col-md-6 col-sm-6 col-xs-12'>
			<a class='btn btn-default form-control' href="<?php echo site_url('alert_dims') ?>">Επιστροφή</a>
		</div>
		<div class='form-group col-lg-6 col-md-6 col-sm-6 col-xs-12'>
