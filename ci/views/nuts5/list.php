	...
	<h2 class='text-center'><?php echo $this->lang->line('nuts5');?></h2>
		...
		<div class='form-group'>
			...
			<div class='col-lg-3 col-md-3 col-sm-3 col-xs-12 text-right'>
				<a class='btn btn-primary form-control' href='<?php echo site_url('nuts5s/add') ?>'><?php echo $this->lang->line('add');?></a>
			</div>
		</div>
		<div class='col-lg-12 col-md-12 col-sm-12 col-xs-12'>
			<table class='table table-hover table-striped'>
				<thead>
					<tr>
            <th><?php echo $this->lang->line('nuts5_name');?></th>
            <th></th>
					</tr>
				</thead>
				<tbody>
					<?php foreach ($viewdata As $key => $val) { ?>
						<tr>
							<td><a href="<?php echo site_url('nuts5s/update/'.$key) ?>" > <?php echo $val ?> </a></td>
			...
			<p class='text-center'>
				<a class='btn btn-primary' href='<?php echo site_url('nuts5s/add') ?>'><?php echo $this->lang->line('add');?></a>
			</p>
			<?php } ?>
		</div>
	</div>
  ...
  <script>
    $(".del-task").click(function(){
      ...
      if(confirm("Θέλετε να διαγράψετε το αντικείμενο "+thetask+";")){
        $.ajax({
         url: "<?php echo site_url('nuts5s/delete/')?>"+"/"+theid,
         type: 'post',
